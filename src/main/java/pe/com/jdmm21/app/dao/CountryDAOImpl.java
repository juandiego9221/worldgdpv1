...
import pe.com.jdmm21.app.model.mapper.CountryRowMapper;
...
public class CountryDAOImpl implements CountryDAO {
	...
	public List<Country> getCountries(Map<String, Object> params) {
		...
		String queryCadena =SELECT_CLAUSE + " WHERE 1=1 "
				+ (!StringUtils.isEmptyOrWhitespaceOnly((String) params.get("search")) ? SEARCH_WHERE_CLAUSE : "")
				+ (!StringUtils.isEmptyOrWhitespaceOnly((String) params.get("continent")) ? CONTINENT_WHERE_CLAUSE : "")
				+ (!StringUtils.isEmptyOrWhitespaceOnly((String) params.get("region")) ? REGION_WHERE_CLAUSE : "")
				+ PAGINATION_CLAUSE;
		return namedParamJdbcTemplate.query(queryCadena, params, new CountryRowMapper());
	}
	...
	public Country getCountryDetail(String code) {
		...
		return namedParamJdbcTemplate.queryForObject(SELECT_CLAUSE + " WHERE c.code = :code", params,
				new CountryRowMapper());
	}

	public void editCountryDetail(String code, Country country) {
		namedParamJdbcTemplate.update(" UPDATE country SET " + " name = :name, " + " localname = :localName, "
				+ " capital = :capital, " + " continent = :continent, " + " region = :region, "
				+ " HeadOfState = :headOfState, " + " GovernmentForm = :governmentForm, " + " IndepYear = :indepYear, "
				+ " SurfaceArea = :surfaceArea, " + " population = :population, " + " LifeExpectancy = :lifeExpectancy "
				+ "WHERE Code = :code ", getCountryAsMap(code, country));
	}
	...
	private static final String SELECT_CLAUSE = "SELECT " + " c.Code, " + " c.Name, " + " c.Continent, " + " c.region, "
			+ " c.SurfaceArea surface_area, " + " c.IndepYear indep_year, " + " c.Population, "
			+ " c.LifeExpectancy life_expectancy, " + " c.GNP, " + " c.LocalName local_name, "
			+ " c.GovernmentForm government_form, " + " c.HeadOfState head_of_state, " + " c.code2 ," + " c.capital ,"
			+ " cy.name capital_name " + " FROM country c" + " LEFT OUTER JOIN city cy ON cy.id = c.capital ";
